...
#include "cmsis_os2.h"     // <--- 添加这一行，用于 osDelay
/* 如果头文件未定义，提供默认值（以 0.91" 128x32 为默认） */
#ifndef OLED_WIDTH
...
#define CPU_FREQ_HZ 17000000

static void delay_us(uint32_t us)
{
    volatile uint32_t ticks = us* (CPU_FREQ_HZ / 1000000);
    while(ticks--);
}

#define OLED_W_SCL(x)		HAL_GPIO_WritePin(GPIOA, GPIO_PIN_15, (x?GPIO_PIN_SET:GPIO_PIN_RESET))
...
void OLED_I2C_SendByte(uint8_t Byte)
{
	...
	for (i = 0; i < 8; i++)
	{
		...
		/*两个!的作用是，让所有非零的值变为1*/
		OLED_W_SDA(!!(Byte & (0x80 >> i)));
		delay_us(1);        // 添加延时，数据建立时间
		OLED_W_SCL(1);	//释放SCL，从机在SCL高电平期间读取SDA
		delay_us(2);        // 添加延时，SCL高电平时间
		OLED_W_SCL(0);	//拉低SCL，主机开始发送下一位数据
		delay_us(1);        // 添加延时，SCL低电平时间
	}
	
	// ACK周期
	OLED_W_SDA(1);      // 释放SDA线，准备读取ACK
	delay_us(1);        // 添加延时
	OLED_W_SCL(1);		//额外的一个时钟，用于ACK
	delay_us(2);        // 添加延时，等待从设备响应
	OLED_W_SCL(0);
	delay_us(1);        // 添加延时
}
...
// 	OLED_WriteCommand(OLED_HEIGHT - 1);	//0x0E~0x3F
...
//     OLED_WriteCommand(0x8F);	//0x00~0xFF
...
void OLED_Init(void)
{
    ...
    osDelay(1);
    OLED_WriteCommand(0xA8);    // 设置多路复用率
    OLED_WriteCommand(OLED_HEIGHT - 1); // 根据屏幕高度设置
    osDelay(1);
    OLED_WriteCommand(0xD3);    // 设置显示偏移
    ...
    OLED_WriteCommand(0x00);    // 列起始地址 = 0
    OLED_WriteCommand(0x7F);    // 列结束地址 = 127
    osDelay(1);
    // 设置页地址范围  
    OLED_WriteCommand(0x22);    // 设置页地址
    OLED_WriteCommand(0x00);    // 页起始地址 = 0
    OLED_WriteCommand(OLED_PAGES - 1); // 页结束地址
    osDelay(1);
    // 扫描方向
    OLED_WriteCommand(0xA1);    // 段重映射，列地址127映射到SEG0
    OLED_WriteCommand(0xC8);    // 输出扫描方向，从COM0到COM[N-1]
    osDelay(1);
    // 硬件配置
    OLED_WriteCommand(0xDA);    // 设置COM引脚硬件配置
    if (OLED_HEIGHT == 32)
        OLED_WriteCommand(0x02); // 32像素高度
    else if (OLED_HEIGHT == 64)
        OLED_WriteCommand(0x12); // 64像素高度
    osDelay(1);
    // 对比度和充电泵设置
    OLED_WriteCommand(0x81);    // 设置对比度
    OLED_WriteCommand(0x8F);    // 对比度值
    osDelay(1);
    OLED_WriteCommand(0xD9);    // 设置预充电周期
    OLED_WriteCommand(0xF1);
    osDelay(1);
    ...
    osDelay(1);
    // 显示设置
    OLED_WriteCommand(0xA4);    // 整个显示跟随RAM内容
...
void OLED_Update(void)
{
	...
	for (j = 0; j < OLED_PAGES; j ++)
	{
		/*设置光标位置为每一页的第一列*/
		OLED_SetCursor(j, 0);
		/*连续写入OLED_WIDTH个数据，将显存数组的数据写入到OLED硬件*/
...
void OLED_UpdateArea(int16_t X, int16_t Y, uint8_t Width, uint8_t Height)
{
	...
	{
		 if (X >= 0 && X <= (OLED_WIDTH - 1) && j >= 0 && j <= (OLED_PAGES - 1))	//超出屏幕的内容不显示
		{
			/*设置光标位置为相关页的指定列*/
			OLED_SetCursor(j, X);
...
{
	uint8_t i, j;
	for (j = 0; j < OLED_PAGES; j ++)				//遍历8页
	{
		for (i = 0; i < OLED_WIDTH; i ++)			//遍历128列
		{
...
void OLED_ClearArea(int16_t X, int16_t Y, uint8_t Width, uint8_t Height)
{
	...
	for (j = Y; j < Y + Height; j ++)		//遍历指定页
	{
		...
		{
            if (i >= 0 && i <= (OLED_WIDTH - 1) && j >=0 && j <= (OLED_HEIGHT - 1))
			//超出屏幕的内容不显示
			{
...
void OLED_ReverseArea(int16_t X, int16_t Y, uint8_t Width, uint8_t Height)
{
	...
	for (j = Y; j < Y + Height; j ++)		//遍历指定页
	{
		for (i = X; i < X + Width; i ++)	//遍历指定列
		{
            if (i >= 0 && i <= (OLED_WIDTH - 1) && j >=0 && j <= (OLED_HEIGHT - 1))			//超出屏幕的内容不显示
			{
...
void OLED_DiagnosticPattern(void)
{
    // 页 0..OLED_PAGES-1
    for (uint8_t p = 0; p < OLED_PAGES; p++) {
        for (uint16_t x = 0; x < OLED_WIDTH; x++) {
            // 模式：奇偶页面不同条纹
            if (p % 4 == 0) OLED_DisplayBuf[p][x] = 0xFF;
            else if (p % 4 == 1) OLED_DisplayBuf[p][x] = (x & 1) ? 0xFF : 0x00;
            else if (p % 4 == 2) OLED_DisplayBuf[p][x] = 0xAA;
            else OLED_DisplayBuf[p][x] = 0x55;
        }
    }
    OLED_Update();
}
...
void OLED_ShowImage(int16_t X, int16_t Y, uint8_t Width, uint8_t Height, const uint8_t *Image)
{
    ...
    for (j = 0; j < (Height - 1) / 8 + 1; j ++)
    {
        ...
        for (i = 0; i < Width; i ++)
        {
            if (X + i >= 0 && X + i <= (OLED_WIDTH - 1))		//超出屏幕的内容不显示
            {
                ...
                if (Page + j >= 0 && Page + j <= (OLED_PAGES - 1))		//超出屏幕的内容不显示
                {
                    /*显示图像在当前页的内容*/
                    OLED_DisplayBuf[Page + j][X + i] |= Image[j * Width + i] << (Shift);
                }
                
                if (Page + j + 1 >= 0 && Page + j + 1 <= (OLED_PAGES - 1))		//超出屏幕的内容不显示
                {					
                    /*显示图像在下一页的内容*/
...
{
    if (X >= 0 && X <= (OLED_WIDTH - 1) && Y >=0 && Y <= (OLED_HEIGHT - 1))
	{
		/*将显存数组指定位置的一个Bit数据置1*/
...
void OLED_TestPattern(void)
{
    ...
    for(uint8_t i = 0; i < OLED_WIDTH; i++) {
        OLED_DisplayBuf[0][i] = 0xFF;  // 第一页全亮
        OLED_DisplayBuf[OLED_PAGES-1][i] = 0xFF; // 最后一页全亮
    }
    OLED_Update();
}
